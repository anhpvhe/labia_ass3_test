...
import lab.ia.ExpenseManagement.Services.RecordService;
...
//    }
    ...
    public ResponseEntity<?> getRecord(@PathVariable(name = "id") Long id,
                                       @CurrentUser UserPrincipal currentUser) {
        return ResponseEntity.ok(recordService.getRecord(id, currentUser));
    }
    ...
    public ResponseEntity<?> getRecordInRange(@RequestParam(name = "page", defaultValue = "0") int page,
                                              ...
                                              @CurrentUser UserPrincipal currentUser) {
        return ResponseEntity.ok(recordService.getRecordsInRange(page, size, startDate, endDate, currentUser));
    }
    ...
    public ResponseEntity<?> addRecord(@RequestBody RecordRequest recordRequest,
        ...
        return ResponseEntity.ok(recordService.addRecord(recordRequest, currentUser));
    }
    ...
    public ResponseEntity<?> updateRecord(@PathVariable(name = "id") Long id,
                                          ...
                                          @CurrentUser UserPrincipal currentUser) {
        return ResponseEntity.ok(recordService.updateRecord(id, recordRequest, currentUser));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<?> deleteRecord(@PathVariable(name = "id") Long id,
                                          @CurrentUser UserPrincipal currentUser) {
        return ResponseEntity.ok(recordService.deleteRecord(id, currentUser));
    }
}
